...
(defn rank [hand]
  ...
  (if-let [face ({\T 10 \J 11 \Q 12 \K 13 \A 14} (first hand))]
    face
    (Integer/valueOf (str (first hand)))))
...
(defn straight? [hand]
  (or (= #{\T \J \Q \K \A} (set (map first hand)))
      (->> hand
           (map rank)
           (replace {14 1})
           (sort)
